		...
		if (keys & KEY_SECONDARY_ATTACK)
		{
			switch (Project[playerid][e_EditType])
			{
				case EDIT_TYPE_POSITION:
				{
					...
					SetTimerEx("ShowProgressBarMenu", 300, false, "dd", playerid, barid);
			...
			SetPlayerProgressBarValue(playerid, barid, 100.0);
...
stock Project_Open(playerid, name[])
{
	...
	for (new i = 0; i < rows; i ++)
	{
		...
		SetPlayerProgressBarValue(playerid, ProjectBars[playerid][i][e_ProgressBarID], 100.0);
...
/*
	...
	Parameters:
		...
		barid - The ID of the player bar.
	Returns:
		1: if the menu was shown.
		0: if the player bar doesn't exist.
*/

forward ShowProgressBarMenu(playerid, barid);
public ShowProgressBarMenu(playerid, barid)
{
	if (Project[playerid][e_ProjectActive])
	{
		new
...
/*
	...
	Parameters:
		...
		barid - The ID of the progress bar.
	Returns:
		The index of "ProjectBars" that contains the progress bar ID.
*/

stock GetProgressBarInternalID(playerid, barid)
{
	for (new i = 0; i < MAX_PLAYER_BARS; i ++)
	{
...
stock DuplicateProgressBar(playerid, index)
{
	...
	{
		new
			barid = ProjectBars[playerid][index][e_ProgressBarID],
			Float:x,
			Float:y;
					...
					case 1: // Export project...
					{
						...
						if (file)
						{
							...
							for (new i = 0; i < MAX_PLAYER_BARS; i ++)
							{
								if (ProjectBars[playerid][i][e_ProgressBarID] != INVALID_PLAYER_BAR_ID)
								{
									format(string, sizeof(string), "new Bar%d[MAX_PLAYERS];\r\n", i);
									fwrite(file, string);
								}
							}
							fwrite(file, "\r\npublic OnPlayerConnect(playerid)\r\n{\r\n");
